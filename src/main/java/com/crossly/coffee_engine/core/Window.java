...
class Window {
    ...
    void setMonitor(Application app) {
        ...
        if (app.isFullscreen()) {
            int[] xpos = new int[1], ypos = new int[1];
            glfwGetMonitorPos(monitor, xpos, ypos);
            glfwSetWindowMonitor(windowHandle, monitor, xpos[0], ypos[0], app.getWidth(), app.getHeight(), mode.refreshRate());
        } else {
            int[] xpos = new int[1], ypos = new int[1];
            glfwGetMonitorPos(monitor, xpos, ypos);
            glfwSetWindowMonitor(windowHandle, 0, xpos[0] + (mode.width() - app.getWidth()) / 2, ypos[0] + (mode.height() - app.getHeight()) / 2, app.getWidth(), app.getHeight(), mode.refreshRate());
            centerOnMonitor(app.getWidth(), app.getHeight(), monitor);
        }
        glViewport(0, 0, app.getWidth(), app.getHeight());
        app.onResize();
