...
public class Entity {
	...
	// Needs optimization
	public final <Comp extends Component> Optional<Comp> getComponentByParentClass(Class<Comp> compType) {
		for (var key : components.keySet()) {
			var superClass = key.getSuperclass();
			do {
				if (superClass == compType)
					return Optional.ofNullable(compType.cast(components.get(key)));
				superClass = superClass.getSuperclass();
			} while (superClass != Component.class);
		}
	...
	public final void destroy() {
		components.forEach((compClass, component) -> component.delete());
		components.clear();
		updateCallback = null;
		renderCallback = null;
	}
